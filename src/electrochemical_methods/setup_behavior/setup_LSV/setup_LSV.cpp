/******************************************************************************
 * @brief: Source file containing the subclass (C_Setup) C_Setup_LSV which 
...
// Include guard
#ifndef setup_LSV_CPP
#define setup_LSV_CPP

// Include dependencies
#include "setup_LSV.h"

/******************************************************************************
 * @brief Constructor of the class C_Setup_LSV
 * 
 *****************************************************************************/ 
C_Setup_LSV::C_Setup_LSV(){}

/******************************************************************************
 * @brief Starting method for the class C_Setup_LSV
 * @param c_DataSoftwareStorage: Reference to data software storage object
 * 
 *****************************************************************************/
int C_Setup_LSV::Begin(C_DataSoftwareStorage * c_DataSoftwareStorage){
...
int C_Setup_LSV::funInitLSV(){
    ...
    if (abs(c_DataStorageLocal_->get_UpperVoltage() - 
         c_DataStorageLocal_->get_StartVoltage()) + 
         AD5940_6BIT_DAC_1LSB >= fRange){
    ...
    if (c_DataStorageLocal_->get_FixedWEPotential() == true){
        ...
        if (c_DataStorageLocal_->get_UpperVoltage() < 
            c_DataStorageLocal_->get_StartVoltage()){
            fWePotentialHigh = c_DataStorageLocal_->get_StartVoltage() + 
        ...
        float fWePotentialHigh = 0.0;
        float fWePotentialLow = 0.0;
        if (c_DataStorageLocal_->get_UpperVoltage() < 
            c_DataStorageLocal_->get_StartVoltage()){
            fWePotentialHigh = c_DataStorageLocal_->get_StartVoltage() + 
                50.0 + AD5940_MIN_DAC_OUTPUT;
            fWePotentialLow = c_DataStorageLocal_->get_UpperVoltage() + 
                50.0 + AD5940_MIN_DAC_OUTPUT;
        }
        else {
            fWePotentialHigh = c_DataStorageLocal_->get_UpperVoltage() + 
                50.0 + AD5940_MIN_DAC_OUTPUT;
            fWePotentialLow = c_DataStorageLocal_->get_StartVoltage() + 
                50.0 + AD5940_MIN_DAC_OUTPUT;
        }
        ...
        if (fWePotentialLow <= AD5940_MIN_DAC_OUTPUT){
            fWePotentialLow = AD5940_MIN_DAC_OUTPUT;
        }
 ...
 * @return: Error code encoded as integer
 *****************************************************************************/
int C_Setup_LSV::funSequencerInitializationSequence(){
    // Initalize variables
 ...
 *****************************************************************************/
int C_Setup_LSV::funSequencerADCControl(){
 ...
 * @return: Returns error code encoded as integer
 *****************************************************************************/
int C_Setup_LSV::funSequencerDACControl(){
    // Initialize variables
    int iDacMaxSeqLen;
