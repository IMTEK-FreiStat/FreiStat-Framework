...
class SetupDPV(SetupBehavior):
    ...
    def setup(self, listExperimentParameters: list) -> int:
        ...
        fMinVoltage: float = (
            listExperimentParameters[0][1] - listExperimentParameters[3][1]
        )
        fMaxVoltage: float = (
            listExperimentParameters[1][1] + listExperimentParameters[3][1]
        )

        # Scan range is limited to 1.9 V / 4.4 V (Depending on the mode)
        fRange: float = fMaxVoltage - fMinVoltage
