"""
Module implementing the behavior of the execute function for measuring the open
circuit potential. 

TODO: This class is not yet fully implemented (10.09.2021)
...
class ExecuteOCP(ExecuteBehavior):
    ...
    def execute(
        ...
        Description
        -----------
        Exectute method for measuring the open circuit potential.
        ...
        if bEnableReading == True:
            ...
            self._handleDataOCP(dataQueue)

        # No error occured
        return EC_NO_ERROR

    def _handleDataOCP(self, dataQueue) -> None:
        """
        Description
        -----------
        ...
        while True:
            ...
            if listReadData[0][0] == ('"' + RUN + '"'):
                ...
                if self._referenceTime == -1:
                    self._referenceTime = float(listReadData[1][1][2][1])
                ...
                if strRun != listReadData[0][1]:
                    ...
                    if self._bPorgressiveMesurement == False:
                        self._referenceTime = float(listReadData[1][1][2][1])
                ...
                fTimeStamp: float = (
                    float(listReadData[1][1][2][1]) - self._referenceTime
                )

                # Add data to data storage
                self._dataHandling.append_StoredData(
                    [int(strRun, 10), iDataPoint, fVoltage, fTimeStamp]
                )

                # Add data to dataQueue
                dataQueue.put([int(strRun, 10), iDataPoint, fVoltage, fTimeStamp])
