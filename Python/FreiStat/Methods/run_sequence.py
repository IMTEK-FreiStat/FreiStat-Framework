...
class Run_Sequence(Run_Electrochemical_Method):
    ...
    def start(
        self, SequenceCycles: float = CYCLE_I, LowPerformanceMode: bool = False
    ) -> str:
        """
        Description
        -----------
        ...
        if len(self._listEcMethod) <= 1:
            self._logger.error(
                'Sequence to short, add more methods via the "Run'
        ...
        if len(self._listEcMethod) > FREISTAT_SEQUENCE_LENGTH:
            self._logger.error(
                "Sequence to long, remove methods. Sequence length "
                + "of "
                + str(len(self._listEcMethod))
        ...
        if self._iSetupFailed != 0:
            self._logger.error(
                "Setup error: sequence.add___ method number "
                + str(self._iSetupFailed)
        ...
        self._dataHandling.move_first_data_object()

        # Loop over all objects
        for iIndex in range(len(self._listEcMethod)):
            # Find experiment parameter for cycle
            listTempParameters = self._dataHandling.get_experiment_parameters()
            ...
            # Append parameters
            listTempExperimentParameters[len(listTempExperimentParameters) - 1].append(
                [self._dataHandling.get_experiment_type(), iCycle, listTempParameters]
            )

            # Move to next object
            self._dataHandling.move_next_data_object()
        ...
        if self._FreiStatMode == FREISTAT_STANDALONE:
            ...
            # Create an object for plotting the data
            self._plotter = Plotter(
                strMethod, listTempExperimentParameters, FREISTAT_BACKEND, self._process
            )
    ...
    def P_DataCollection(
        ...
        for iPosition in range(len(self._listEcMethod)):
            ...
            self._listEcMethod[iPosition].execute(
                dataQueue,
                event=self._event,
                iTelegrams=2,
    ...
    def add_CV(
        ...
        # Creating an object for general electrochemical methods
        self._listEcMethod.append(ElectrochemicalMethod(CV, self._dataSoftwareStorage))

        # Execute setup for electrochemical methods
        iErrorCode = self._listEcMethod[len(self._listEcMethod) - 1].setup(
            listTempExperimentParameters
        )
    ...
    def add_LSV(
        ...
        # Creating an object for general electrochemical methods
        self._listEcMethod.append(ElectrochemicalMethod(LSV, self._dataSoftwareStorage))
    ...
    def add_CA(
        ...
        Pulse_Lengths: list = [PULSE_LENGTH_F, PULSE_LENGTH_F, PULSE_LENGTH_F],
        ...
        for iSteps in range(len(Potential_Steps)):
            listPotential_Steps.append(
                self._check_StartingPotential(Potential_Steps[iSteps] * 1000.0, 1)
            )
        ...
        # Creating an object for general electrochemical methods
        self._listEcMethod.append(ElectrochemicalMethod(CA, self._dataSoftwareStorage))
    ...
    def add_NPV(
        ...
        Pulse_Lengths: list = [PULSE_LENGTH_F, PULSE_LENGTH_F],
        ...
        fBaseVoltage = self._check_StartingPotential(BaseVoltage, iFixedWEPotential)
        ...
        # Creating an object for general electrochemical methods
        self._listEcMethod.append(ElectrochemicalMethod(NPV, self._dataSoftwareStorage))
    ...
    def add_DPV(
        ...
        # Creating an object for general electrochemical methods
        self._listEcMethod.append(ElectrochemicalMethod(DPV, self._dataSoftwareStorage))
    ...
    def add_SWV(
        ...
        # Creating an object for general electrochemical methods
        self._listEcMethod.append(ElectrochemicalMethod(SWV, self._dataSoftwareStorage))
