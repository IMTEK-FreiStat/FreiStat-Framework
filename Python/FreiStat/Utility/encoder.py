...
def _encode_Sinc_Oversampling_Rate(
    strFilterName: str, iOversampling: int, logger: logging.Logger
) -> int:
    """
    Description
    -----------
    ...
    if iOversampling < 0:
        logger.warning(
            "Oversampling rate is negative." + " Using positive value instead"
        )
    ...
    if strFilterName == "Sinc2":
        ...
        if iOversampling < ADCSINC2OSR_22_VALUE:
            logger.warning(
                "Oversampling rate below lower bound." + " Oversampling rate set to 22."
            )
            return ADCSINC2OSR_22

        elif iOversampling > ADCSINC2OSR_1333_VALUE:
            logger.warning(
                "Oversampling rate above uppper bound."
        ...
        if iOversampling < ADCSINC3OSR_DISABLED_VALUE:
            logger.warning(
                "Oversampling rate below lower bound." + " Disable Sinc3 filter."
            )
    ...
    for i in range(len(listOversamplingRates) - 1):
        if (
            iOversampling >= listOversamplingRates[i]
            and iOversampling <= listOversamplingRates[i + 1]
        ):
...
def _encode_LPTIA_Resistor_Size(iLPTiaResistorSize: int, logger: logging.Logger) -> int:
    """
    Description
    -----------
    ...
    if iLPTiaResistorSize < 0:
        logger.warning(
            "LPTIA resistor value is negative." + " Using positive value instead"
        )
    ...
    if iLPTiaResistorSize < LPTIARTIA_200R_VALUE:
        logger.warning(
            "LPTIA resistor value below lower bound."
        ...
        logger.warning(
            "LPTIA resistor value above uppper bound."
    ...
    for i in range(len(listLPTiaResistorSizes) - 1):
        if (
            iLPTiaResistorSize >= listLPTiaResistorSizes[i]
            and iLPTiaResistorSize <= listLPTiaResistorSizes[i + 1]
        ):
